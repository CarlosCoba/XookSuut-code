...
def bayesian_mcmc(galaxy, shape, vel_map, e_vel_map, guess, vmode, config, rings_pos, ring_space, steps, e_ISM = 5, pixel_scale = 1, r_bar_min = None, r_bar_max = None):
	...
	def model(theta):
		...
		Vrot,pa,inc,x0,y0,Vsys = theta[:n_circ],theta[m],theta[m+1],theta[m+2],theta[m+3],theta[m+4]
		...
		def eval_mdl(xy_mesh,pa,inc,x0,y0,Vsys, r_0 = None, r_space = ring_space,ii=0):
			...
			circ = CIRC_MODEL(xy_mesh,Vrot_,pa,inc,x0,y0)
	...
	theta = vrot0,pa0,inc0,X0,Y0,vsys0
	theta0 = np.concatenate([x.ravel() for x in theta])
	ndim = len(theta0)

	step_size = np.array([np.ones(n_circ)*1e1,1,1,1e-1,1e-1,1])*1e-1
	...
	used_axis = nrows*ncols - (n_circ + 5)	
	if used_axis < nrows*ncols:	
		for i in range(used_axis):
			globals()['ax_remove%s' % i] = axes[n_circ+5+i]
	...
	nlabels = len(labels)
	flat_samples = chain[:,m:m+5]


	if n_circ < 20:
		fig = corner.corner(flat_samples, labels=labels, truths=[pa0, inc0, X0, Y0, vsys0]);
	...
	Std_errors = eVrot,0*eVrot,epa,einc,ex0,ey0,eVsys,0,0*eVrot

	return chain,vlos_2D_model, kin_2D_models, Vrot, Vrot*0, Vsys,  pa, inc , x0, y0, Vrot*0, 0, Std_errors
