...
def bayesian_mcmc(galaxy, shape, vel_map, e_vel_map, guess, vmode, config, rings_pos, ring_space, steps, e_ISM = 5, pixel_scale = 1, r_bar_min = None, r_bar_max = None):
	...
	def model(theta):
		...
		Vrot, Vrad, pa,inc,x0,y0,Vsys = theta[:n_circ],theta[n_circ:n_circ+n_noncirc],theta[m],theta[m+1],theta[m+2],theta[m+3],theta[m+4]
		...
		interp_model = np.zeros((ny,nx))
		def eval_mdl(xy_mesh,pa,inc,x0,y0,Vsys, r_0 = None, r_space = ring_space,ii=0):

			Vrot_ = Vrot_dic["Vrot%s"%ii]
			if ii + 1 <= n_noncirc:
				Vrad_ = Vrad_dic["Vrad%s"%ii]
				rad = RADIAL_MODEL(xy_mesh,Vrot_,Vrad_,pa,inc,x0,y0)
			else:
				rad = RADIAL_MODEL(xy_mesh,Vrot_,0,pa,inc,x0,y0)
		...
		for N in range(n_annulus):
			...
			for kk in range(2):
				Vxy,Vsys = eval_mdl(XY, pa,inc,x0,y0,Vsys, r_0 = rings_pos[N], r_space = r_space_k, ii = N+kk)
				...
				if N == 0 and kk == 0:
					...
					Vxy,Vsys  = eval_mdl(XY1, pa,inc,x0,y0,Vsys, r_0 = 0, r_space = r_space_0, ii = 0)
		...
		mask_res = abs(res) < 500

		return model_array[mask_res], obs_array[mask_res], error_array[mask_res], Xpos,Ypos
	...
	def ln_prior(theta):
		...
		if  0 <= pa <= 360 and 0 < inc < 90 and 0 < x0 < nx and 0 < y0 < ny and 0 < Vsys < 3e5:
			mask_1 = (Vrot_arr > -400) & (Vrot_arr < 400) 
			mask_2 = (Vrad_arr > -300) & (Vrad_arr < 300)
			if False in mask_1 or False in mask_2:
				return -np.inf
			else:  
				return 0.0
		else:
	...
	theta = vrot0,vrad0,pa0,inc0,X0,Y0,vsys0
	theta0 = np.concatenate([x.ravel() for x in theta])

	ndim = len(theta0)


	#step_size = np.array([np.ones(n_circ)*1e2,np.ones(n_noncirc)*1e2,1,1,1,1,1])*1e-2
	step_size = np.array([np.ones(n_circ)*1e1,np.ones(n_noncirc)*1e1,1,1,1e-1,1e-1,1])*1e-1
	...
	#
	labels = ["$\mathrm{\phi^{\prime}}$", "$i$", "$\mathrm{XC}$", "$\mathrm{YC}$", "$\mathrm{Vsys}$"]
	nlabels = len(labels)
	flat_samples = chain[:,m:m+6]


	fig = corner.corner(
	    flat_samples, labels=labels, truths=[pa0, inc0, X0, Y0, vsys0]
	);
	...
	if n_circ < 20:
		...
		plt.savefig("./plots/corner.vcirc.%s_model.%s.pdf"%(vmode,galaxy))
		plt.tight_layout()
		plt.clf()
		...
		plt.savefig("./plots/corner.vrad.%s_model.%s.pdf"%(vmode,galaxy))
		plt.tight_layout()
		plt.clf()
	...
	Vrot, Vrad, pa,inc,x0,y0,Vsys = medians[:n_circ],medians[n_circ:n_circ+n_noncirc],medians[m],medians[m+1],medians[m+2],medians[m+3],medians[m+4]
	eVrot, eVrad, epa,einc,ex0,ey0,eVsys = errors[:n_circ],errors[n_circ:n_circ+n_noncirc],errors[m],errors[m+1],errors[m+2],errors[m+3],errors[m+4]
	...
	vlos_2D_model = best_2d_model(vmode, shape,V_k, pa, inc, x0, y0, Vsys, rings_pos, ring_space = ring_space, phi_b = 0, pixel_scale = pixel_scale) 
