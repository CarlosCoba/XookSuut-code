...
def circ_mod(galaxy, vel, evel, guess0, vary, n_it, rstart, rfinal, ring_space, frac_pixel, delta, pixel_scale, bar_min_max, errors, config, e_ISM, steps, method, use_metropolis):
		...
		for it in np.arange(n_it):
			...
			guess = [vrot_tab,vrot_tab*0,pa0,inc0,x0,y0,vsys0, vrot_tab*0,theta_b]

			v_2D_mdl, kin_2D_modls, vrot , vsys0,  pa0, inc0, x0, y0, xi_sq, n_data, Errors = fit(shape, vel, evel, guess, vary, vmode, config, R_pos, fit_method = method, e_ISM = e_ISM, pixel_scale = pixel_scale, ring_space = ring_space  )
		...
		if errors == 1:
			...
			from metropolis_hastings_circ import bayesian_mcmc
			...
			chain,v_2D_mdl_, kin_2D_models_, Vrot_, Vrad_, Vsys_,  PA_, INC_ , XC_, YC_, Vtan_, THETA_, std_errors = bayesian_mcmc(galaxy, shape, vel, evel, GUESS, vmode, config, Rings, e_ISM = e_ISM, pixel_scale = pixel_scale, ring_space = ring_space, r_bar_min = 0, r_bar_max = 0, steps = steps)
		...
		return PA,INC,XC,YC,VSYS,0,Rings,Vrot,0*Vrot,0*Vrot,best_vlos_2D_model,best_kin_2D_models,chisq_global,std_errors
