...
class Circular_model:
	...
	def lsq(self):
		...
		for it in np.arange(self.n_it):
			...
			vrot_tab, vrad_tab, vtan_tab, R_pos = tab_mod_vels(self.rings,self.vel, self.evel, self.pa0,self.eps0,self.x0,self.y0,self.vsys0,self.theta_b,self.delta,self.pixel_scale,self.vmode,self.shape,self.frac_pixel,0, 0)
			...
			if xi_sq < self.chisq_global:
				...
				self.GUESS = [self.Vrot, self.Vrot*0, self.Vrot*0, self.PA, self.EPS, self.XC, self.YC, self.VSYS, 0]

				self.bootstrap_kin = np.zeros((self.n_boot, 1*self.n_circ))
	...
	def __call__(self):
		out = self.output()
		return self.PA,self.EPS,self.XC,self.YC,self.VSYS,0,self.Rings,self.Vrot,0*self.Vrot,0*self.Vrot,self.best_vlos_2D_model,self.best_kin_2D_models,self.aic_bic,self.std_errors
