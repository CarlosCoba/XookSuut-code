...
def save_vlos_model(galaxy,vmode,vel_map,evel_map,vlos_2D_model, kin_2D_models,PA,INC,XC,YC,VSYS,m_hrm=None, theta = False, phi_bar_major = False, phi_bar_minor = False):
	...
	if True:
		...
		hdu.writeto("./models/%s.%s.2D_vlos_model.fits.gz"%(galaxy,vmode),overwrite=True)
		...
		hdu.writeto("./models/%s.%s.residual.fits.gz"%(galaxy,vmode),overwrite=True)
		...
		hdu.writeto("./models/%s.%s.chisq.fits.gz"%(galaxy,vmode),overwrite=True)
		...
		hdu.writeto("./models/%s.%s.2D_R.fits.gz"%(galaxy,vmode),overwrite=True)
		...
		if "hrm" not in vmode:			
			hdu.writeto("./models/%s.%s.2D_circ_model.fits.gz"%(galaxy,vmode),overwrite=True)
		...
		if vmode == "radial":
			...
			hdu.writeto("./models/%s.%s.2D_rad_model.fits.gz"%(galaxy,vmode),overwrite=True)
		...
		if vmode == "bisymmetric":
			...
			hdu.writeto("./models/%s.%s.2D_tan_model.fits.gz"%(galaxy,vmode),overwrite=True)
		...
		if "hrm" in vmode:
			...
			for k in range(1,m_hrm+1):
				...
				hdu.header['XC'] = XC
				hdu.header['YC'] = YC			
				hdu.writeto("./models/%s.%s.2D_C%s_model.fits.gz"%(galaxy,vmode,k),overwrite=True)
				...
				hdu.header['XC'] = XC
				hdu.header['YC'] = YC			
				hdu.writeto("./models/%s.%s.2D_S%s_model.fits.gz"%(galaxy,vmode, k),overwrite=True)
