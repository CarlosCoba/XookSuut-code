...
class Least_square_fit:
	def __init__(self,vel_map, e_vel_map, guess, vary, vmode, config, rings_pos, ring_space, fit_method, e_ISM, pixel_scale, frac_pixel, v_center, m_hrm = 1, N_it = 1, boots_ana = True):
...
class Fit_kin_mdls(Models):
		def residual(self, pars):
			...
			mask = interp_model == 0
			interp_model[~mask]  = interp_model[~mask] + Vsys
		...
		def results(self):
			...
			if self.boots_ana :
				return 0, 0, self.V_k, pa, eps , x0, y0, Vsys, phi_b, [0], [0], [0]
