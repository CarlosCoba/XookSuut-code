...
def save_model(galaxy,vmode,R,Vrot,Vrad,Vtan,PA,INC,XC,YC,VSYS,THETA,PA_BAR_MAJOR,PA_BAR_MINOR,errors_fit,bic_aic, e_ISM, out):
	...
	e_Vrot,e_Vrad,e_Vtan,e_PA,e_INC,e_XC,e_YC,e_Vsys,e_theta = errors_fit
	N_free, N_nvarys, N_data, bic, aic, redchi = bic_aic

	e_centroid = e_ISM/np.sin(INC*np.pi/180)
	e_Vrot = np.sqrt( e_Vrot**2 + e_centroid**2 )

	if vmode in ["radial","bisymmetric"]:
		e_Vrad[e_Vrad!=0] =  np.sqrt( e_Vrad**2 + e_centroid**2 )[e_Vrad!=0]
		if np.size(e_Vtan) == 1: e_Vtan = e_Vrad 
		e_Vtan[e_Vrad!=0] =  np.sqrt( e_Vtan**2 + e_centroid**2 )[e_Vrad!=0]
