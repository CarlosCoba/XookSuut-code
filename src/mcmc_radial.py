...
class KinModel :
	...
	def __init__(self, vel_map, evel_map, theta0, vmode, rings_pos, ring_space, pixel_scale, inner_interp, lnsigma_int):
		...
		self.vrot0,self.vrad0,self.pa0,self.inc0,self.xc0,self.yc0,self.vsys0 = theta0
		...
		self.m = self.n_circ + self.n_noncirc
	...
	def interp_model(self, theta ):
		...
		#interp_model = deconv_2D(interp_model, 2.6, 6, 1)
...
from chain_rad import chain_res_mcm
from metropolis_hastings_algo import run_metropolis_hastings
import emcee
prng =  np.random.RandomState(12345)


ln_prob = set_like("radial",KinModel)
