...
class KinModel :
	def __init__(self, vel_map, evel_map, theta_ls, vmode, ring_pos, ring_space, pixel_scale, inner_interp, pdf, m_hrm, n_circ, n_noncirc, shape ):
	...
	def expand_theta(self, theta):
			...
			if "hrm" in self.vmode:
				self.m2_hrm = int(2*self.m_hrm)
				# Here are all the noncircular velocities
				V_k_noncirc = V_k[self.n_circ:]
				# First fill C_k and S_k with zeros 
				self.C_k, self.S_k = [[0 for k in range(self.n_circ)] for k in range(self.m_hrm)], [[0 for k in range(self.n_circ)] for k in range(self.m_hrm)]
				#C_k[0] will always exist
				self.C_k[0] = V_k[:self.n_circ]
				if self.m_hrm !=1:
						for k in range(1,self.m_hrm):
							(self.C_k[k])[:self.n_noncirc] = V_k_noncirc[(k-1)*self.n_noncirc:k*self.n_noncirc]
						for k in range(self.m_hrm,2*self.m_hrm):
							(self.S_k[k-self.m_hrm])[:self.n_noncirc] = V_k_noncirc[(k-1)*self.n_noncirc:k*self.n_noncirc]
				else:
					self.S_k[0][:self.n_noncirc] = V_k_noncirc
			...
			#self.interp_model_r = np.zeros((ny,nx))
