...
from mcmc_radial import Metropolis
...
class Radial_model:
	...
	def mcmc(self):
		...
		if self.stepsize.size == 0:
			#default
			sigmas = np.array([np.ones(n_circ),np.ones(n_noncirc),1,1,1,1,1])*1e-3

		else:
			#custom
			sigmas = np.array([np.ones(n_circ),np.ones(n_noncirc),1,1,1,1,1])*self.stepsize
