...
def chain_res_mcm(galaxy, vmode, sampler, theta, chain, step_size, steps, thin, burnin, accept_rate, shape, save_plots, rings_pos, ring_space, pixel_scale, inner_interp, lnsigma_int, phi_b = 0 ):
	...
	if lnsigma_int == False:
		vrot0,vrad0,vtan0,pa0,inc0,x0,y0,vsys0,phi0 = theta
		labels_const = ["$\mathrm{\phi^{\prime}}~(^\circ)$", "$i~(^\circ)$", "$\mathrm{x_0}~(pix)$", "$\mathrm{y_0}~(pix)$", "$\mathrm{Vsys~(km/s)}$","$\\phi_{\mathrm{bar}}~(^\circ)$"]

		truths_const = [pa0, inc0, x0, y0, vsys0,phi0]
	else:
		vrot0,vrad0,vtan0,pa0,inc0,x0,y0,vsys0,phi0,lnsigma2 = theta
		labels_const = ["$\mathrm{\phi^{\prime}}$", "$i$", "$\mathrm{x_0}$", "$\mathrm{y_0}$", "$\mathrm{Vsys}$","$\\phi_{\mathrm{bar}}$", "$\mathrm{sigma}$"]
	...
	if save_plots == True:
		...
		axes[-1].set_xlabel("$\mathrm{steps}$", fontsize = 10);
		plt.savefig("./plots/chain_progress.rad.%s_model.%s.png"%(vmode,galaxy), dpi=300)
		plt.clf()
		...
		axes[-1].set_xlabel("$\mathrm{steps}$", fontsize = 10);
		plt.savefig("./plots/chain_progress.tan.%s_model.%s.png"%(vmode,galaxy), dpi=300)
		plt.clf()
	...
	V_k = [Vrot, Vrad, Vtan] 
	create_2D = best_2d_model(vmode, shape,V_k, pa, inc, x0, y0, Vsys, rings_pos, ring_space, pixel_scale, inner_interp, phi_b = phi_b) 
	...
	errors = [eVrot,eVrad,eVtan,epa,einc,ex0,ey0,eVsys,ephi_b]
