...
def chain_res_mcmc(galaxy, vmode, theta, mcmc_outs, shape, rings_pos, ring_space, pixel_scale, inner_interp, phi_b = 0, outdir = False, config_psf=None, plot_chain = False ):
	...
	samples_copy[:,m+1],samples_copy[:,m+5] = eps_2_inc(samples_copy[:,m+1])*180/np.pi, samples_copy[:,m+5]*180/np.pi
	...
	for i in range(ndim):
		...
		#
		param = samples[:, i]
		mcmc = np.nanpercentile(param, [2.275, 15.865, 50, 84.135,97.725]); median = mcmc[2]
	...
	pa,eps,x0,y0,Vsys,phi_b = medians[m:m+6]
	epa,eeps,ex0,ey0,eVsys,ephi_b = std[m:m+6]
	...
	e_constant_parms = [epa,eeps,ex0,ey0,eVsys,ephi_b]
	...
	marginal_vals(galaxy,vmode,chain_res_copy,n_circ,n_noncirc,outdir,nlabels,mcmc_outs[1:-1])

	return vlos_2D_model, kin_2D_models, V_k, pa, eps, x0, y0, Vsys, phi_b, errors
