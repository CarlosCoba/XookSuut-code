import numpy as np
from convolution import deconv_2D
...
class KinModel :
	...
	def __init__(self, vel_map, evel_map, theta0, vmode, rings_pos, ring_space, pixel_scale, inner_interp, lnsigma_int):
		...
		self.vrot0,self.pa0,self.inc0,self.xc0,self.yc0,self.vsys0 = theta0
	...
	def interp_model(self, theta ):
		...
		interp_model = np.zeros((self.ny,self.nx))*np.nan
...
from chain_circ import chain_res_mcm
from metropolis_hastings_algo import run_metropolis_hastings
ln_prob = set_like("circular",KinModel)
import emcee
